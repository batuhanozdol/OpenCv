...
import cv2
 # multiscale (blue,greeen,red) BGR
...
smile = cv2.CascadeClassifier('C:/Users/CEM/Desktop/haarcascade_smile.xml')
def detect(gray, frame): # We create a function that takes as input the image in black and white (gray) and the original image (frame), and that will return the same image with the detector rectangles. 
    ...
    for (x, y, w, h) in faces: # For each detected face:
        ...
        eyes = eye_cascade.detectMultiScale(roi_gray, 1.1, 4) # We apply the detectMultiScale method to locate one or several eyes in the image.
        for (ex, ey, ew, eh) in eyes: # For each detected eye:
            cv2.rectangle(roi_color,(ex, ey),(ex+ew, ey+eh), (0, 255, 0), 2) # We paint a rectangle around the eyes, but inside the referential of the face
        # for smile
        smiles = smile.detectMultiScale(roi_gray, 1.7, 20) 
        for (ax, ay, aw, ah) in smiles: # For each detected eye:
...
while True: # We repeat infinitely (until break):
    ...
    if cv2.waitKey(1) & 0xFF == ord('q'): # If we type on the keyboard: quit for pressing q
